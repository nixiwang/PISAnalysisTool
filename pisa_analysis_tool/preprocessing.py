...
def add_lat_long(filedir, country_dict):
    ...
    :param country_dict: a dictionary which keys are country names
                         and values are [lat, lon]
    :return: new csv file and a list of countries which
             have PISA test score
    """
    new_file = dict()
...
def remove_country(csv_dir, country_dict, country_lst):
    """
    extract the countries which have gender parity index and also
    in world country data
    :param csv_dir: a csv file which have gender coefficent.
    :param country_dict: a dictionary which keys are country name
                         and value are (lat, lon)
